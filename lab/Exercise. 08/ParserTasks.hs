...
-- 123
digitListToInteger :: [Int] -> Int
digitListToInteger = read . concat . map show
...
numberParser = do
  xs <- digitsListParser
  succeed $ digitListToInteger xs

digitsListParser :: Parser [Int]
digitsListParser = do
  digit <- digitParser
  mdigits <- optional digitsListParser
  case mdigits of
    Nothing -> succeed [digit]
    Just digits -> succeed $ digit : digits
