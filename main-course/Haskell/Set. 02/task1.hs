...
revOneLine :: Int -> Int
revOneLine n = read $ reverse $ show n -- => cast the input number to string, reverse the string, convert it to number
...
primeOneLineLC n = [x | x <- [1 .. n], mod n x == 0] == [1, n]

primeOneLineHOF :: Int -> Bool
-- primeOneLineHOF n = n > 1 && (null $ filter (\x -> mod n x == 0) [2 .. n - 1])
primeOneLineHOF n = filter (\x -> mod n x == 0) [1 .. n] == [1, n]